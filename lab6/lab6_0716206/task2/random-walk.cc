...
void Movement (Ptr<Node> node, std::vector<Vector> bound, FILE *fp, double interval, int cnt, MOVE M);
...
int main (int argc, char *argv[])
{
  ...
  // simulation setup
  wifiPhy.EnablePcap ("random-walk", wirelessDevices);
  ...
  MOVE M;
  Simulator::Schedule (Seconds(interval/10), &Movement, wirelessNodes.Get(0), GetBoundary(fp), fp, interval, 0, M);
...
void Movement (Ptr<Node> node, std::vector<Vector> bound, FILE *fp, double interval, int cnt, MOVE move)
{
  // [TODO]:
  ...
  if( cnt == 0 ){
    move = GetSingleMovement(fp);
  }

  Ptr<MobilityModel> mob = node->GetObject<MobilityModel>();
  Vector pos = mob->GetPosition();
  double x = pos.x + cos(move.rad) * move.speed * interval;
  double y = pos.y + sin(move.rad) * move.speed * interval;

  if( cnt == 0 ){
    while( x > bound[0].x || x < bound[1].x || y > bound[0].x || y < bound[1].y ){
      // illegal
      PrintIllegalMsg(move.index);
      move = GetSingleMovement(fp);
      x = pos.x + cos(move.rad) * move.speed * interval;
      y = pos.y + sin(move.rad) * move.speed * interval;
    }
  }

 
  // legal
  pos.x = pos.x + cos(move.rad) * move.speed * (interval/10); // move smoothly
  pos.y = pos.y + sin(move.rad) * move.speed * (interval/10);
  mob->SetPosition(pos);
  ...
  Simulator::Schedule (Seconds(interval/10), &Movement, node, bound, fp, interval, cnt, move);
}
