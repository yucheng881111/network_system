...
void packetHandler(u_char * arg, const struct pcap_pkthdr * pkthdr, const u_char * packet){
    ...
    */
    ...
    if (ntohs(ethernetHeader->ether_type) == 2048) {
      ...
      // ip link add GRE1 type gretap remote 140.114.0.1 local 140.113.0.2 key 1;
      // ip link add GRE2 type gretap remote 140.114.0.1 local 140.113.0.2 key 2;
      // ip link set GRE1 up
      // ip link set GRE2 up
      // ip link add br1 type bridge
      // brctl addif br1 BRGr-eth0
